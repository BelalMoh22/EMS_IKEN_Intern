...
import { useAuthStore } from "@/stores/auth"; // Gives current URL info
...
export function ProtectedRoute({ children }: { children }) {
  ...
  if (!isAuthenticated) return <Navigate to="/login" replace />; // replace : Don't keep previous page in history
