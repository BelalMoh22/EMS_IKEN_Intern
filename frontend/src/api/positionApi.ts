...
export const positionApi = {
  ...
  getById: (id: number) =>
    api.get<ApiResponse<Position>>(`/positions/${id}`).then((r) => r.data.data!),
  ...
  update: (id: number, data: UpdatePositionRequest) =>
    api.put<ApiResponse<void>>(`/positions/${id}`, data).then((r) => r.data.data!),

  delete: (id: number) =>
    api.delete<ApiResponse<void>>(`/positions/${id}`).then((r) => r.data.data!),
};
