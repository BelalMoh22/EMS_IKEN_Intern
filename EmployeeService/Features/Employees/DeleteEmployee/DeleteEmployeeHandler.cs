namespace EmployeeService.Features.Employees.DeleteEmployee
{
    public class DeleteEmployeeHandler : IRequestHandler<DeleteEmployeeCommand, EmployeeActionResult>
    {
        ...
        public async Task<EmployeeActionResult> Handle(DeleteEmployeeCommand request, CancellationToken cancellationToken)
        {
            ...
            var deptNames = await _rules.HandleManagerRemovalAsync(request.id);
            ...
            if (deptNames.Any())
            {
                message += $" Note: Manager '{employee.FirstName} {employee.Lastname}' was removed from departments: {string.Join(", ", deptNames)}. Please assign a new manager.";
            }
