namespace backend.Features.Attendance.MyAttendance
{
        ...
        : IRequestHandler<GetMyAttendanceQuery, MyAttendanceResultDto>
    {
        ...
        public GetMyAttendanceHandler(AttendanceRepository attendanceRepo,EmployeeRepository employeeRepo,ILogger<GetMyAttendanceHandler> logger)
        {
        ...
        public async Task<MyAttendanceResultDto> Handle(GetMyAttendanceQuery request,CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation("Fetched {Count} attendance records for Employee {EmployeeId} (Filters: Y:{Year}, M:{Month}, D:{Day}).",result.Records.Count, employee.Id, request.Year, request.Month, request.Day);
